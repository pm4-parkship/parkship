...
/**
 * Controller for signing up and signing in resp. for the registration
 * and the login of a normal user.
 */
@RestController
@RequestMapping("/auth")
public class AuthenticationController {
    /**
     * Record for having the users request from the frontend to sign in
     * in one clean object.
    ...
    /**
     * Record for having the response to a users sign in request in
     * one clean object.
    ...
    /**
     * Record for having the users request from the frontend to sign up
     * in one clean object.
     * @param email
     * @param username
     * @param password
    ...
    /**
     * Record for having the response to a users sign up request in one
     * clean object.
     * @param id
     * @param username
     * @param roles
    ...
    /**
     * Processes a users request to sign up. If the user already exists,
     * it throws an error and if not, it registers the user and sends a
    ...
    public SignUpResponseDTO signUp(@Valid @RequestBody SignUpRequestDTO signUpRequestDTO) {
        ...
        ApplicationUser newUser = userService.signUp(signUpRequestDTO.username, signUpRequestDTO.email, signUpRequestDTO.password);
        return new SignUpResponseDTO(newUser.getId(), newUser.getUsername(), newUser.getRoles().stream().map(Role::getName).collect(Collectors.toSet()));
    }

    /**
     * Processes a users request to sign in. First it authenticates the user,
     * if the user is not authenticated, it throws an error and if not, it
    ...
    public SignInResponseDTO signIn(@Valid @RequestBody SignInRequestDTO signInRequestDTO) {
        ...
        return new SignInResponseDTO(token, user.getUsername(), user.getRoles().stream().map(Role::getName).collect(Collectors.toSet()));
    }
}
