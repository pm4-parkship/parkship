...
import {TagbarProps, TagData} from "./tag-bar";
...
const TagMenu = ({options, selected, addTag}: TagMenuProps) => {
    ...
    return <div>
        <li>
            <IconButton aria-label="delete" onClick={handleClickListItem} size="small">
                <AddIcon/>
            </IconButton>
        </li>
        <Menu
        ...
        >
            {options.filter((value) => !selected.includes(value)).map((option) => (
                <MenuItem
                    key={option.key}
